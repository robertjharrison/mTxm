...
def print_16n_tuner(indent,n,data):
    print(indent*tab,"else if (ni>32 && nj<=%d) {" % n);
    indent += 1
    print_list(indent,"ilist",[data[256,j,16][0] for j in range(1,n+1)])
    print_list(indent,"jlist",[data[256,j,16][1] for j in range(1,n+1)])
    print(indent*tab,"itile=ilist[nj]; jtile=jlist[nj];")
    indent -= 1
    print(indent*tab,"}")

def print_16nT_tuner(indent,n,data):
    print(indent*tab,"else if (ni<=%d && nj>32) {" % n);
    indent += 1
    print_list(indent,"ilist",[data[i,256,16][0] for i in range(1,n+1)])
    print_list(indent,"jlist",[data[i,256,16][1] for i in range(1,n+1)])
    print(indent*tab,"itile=ilist[ni]; jtile=jlist[ni];")
    indent -= 1
    print(indent*tab,"}")
...
def do_16n(indent,f):
    ...
    n = int(line.split()[1])
    data = parse(f,n)
    print_16n_tuner(indent,n,data)

def do_16nT(indent,f):
    line = f.readline()
    n = int(line.split()[1])
    data = parse(f,n)
    print_16nT_tuner(indent,n,data)
